...
public class Trasferimento implements Serializable {
    ...
    private static File getCsvFile() {
        ...
        File found = searchSubtreeForCsv(current, 4);
        if (found != null) {
            return found;
        }

        found = searchParentPathsForCsv(current, 4);
        if (found != null) {
            return found;
        }
        ...
        try {
            ...
            File projectRoot = base.getName().equals("bin") ? base.getParentFile() : base;
            found = searchSubtreeForCsv(projectRoot, 4);
            if (found != null) {
                return found;
            }
    ...
    private static File searchParentPathsForCsv(File start, int maxLevels) {
        File current = start;
    ...
    private static File searchSubtreeForCsv(File start, int maxDepth) {
        if (start == null || maxDepth < 0) {
            return null;
        }
        File csvFolder = new File(start, CSV_DIR);
        File candidate = new File(csvFolder, CSV_FILE_NAME);
        if (candidate.exists()) {
            return candidate;
        }
        if (maxDepth == 0) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                File found = searchSubtreeForCsv(file, maxDepth - 1);
                if (found != null) {
                    return found;
                }
            }
